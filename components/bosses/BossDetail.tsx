...
export function BossDetail() {
  ...
  const state = useEntity<{ data: Boss }>("bosses", slug as string);

  const isLoading = state.status === "loading" || state.status === "idle";
  const isError = state.status === "error";
  const boss = state.status === "success" ? state.data.data : null;
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center gap-6">
        <div className="w-16 h-16 rounded-full border border-ds-border flex items-center justify-center text-ds-muted">
          <HugeiconsIcon icon={Sword} size={32} />
        </div>
        ...
        <div className="lg:col-span-2 flex flex-col gap-8">
          ...
          <div className="relative group overflow-hidden rounded-sm border border-ds-border bg-ds-surface/40 p-8 sm:p-12">
            ...
            <div className="relative flex flex-col sm:flex-row items-center gap-8">
              {boss.imageUrl && (
                <div className="relative w-32 h-32 sm:w-64 sm:h-64 rounded-sm overflow-hidden border border-ds-border-2 shadow-2xl">
                  <Image
                    src={boss.imageUrl}
                    alt={boss.name}
                    fill
              ...
              <div className="flex flex-col items-center sm:items-start text-center sm:text-left gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <span className={cn(
                    "text-[10px] uppercase tracking-tighter flex items-center gap-1 border px-2 py-0.5 rounded-sm bg-ds-surface/50 font-display",
                    boss.isOptional
                      ? "text-ds-gold-dim border-ds-gold-dim/30"
                      : "text-red-500/80 border-red-900/50"
                  )}>
                    {boss.isOptional ? t("details.optional") : t("details.mandatory")}
                  </span>
                </div>
        ...
        <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-right-4 duration-1000">
          <div className="rounded-sm border border-ds-border bg-ds-surface/30 p-6 flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-5">
              ...
              {boss.healthPoints && (
                <div className="flex flex-col gap-2">
                  <div className="flex items-center gap-2 text-[10px] uppercase tracking-widest text-ds-muted font-display">
                    <HugeiconsIcon icon={FavouriteIcon} size={14} className="text-red-500/70" />
                    {t("details.health")}
                  </div>
                  <span className="text-sm text-ds-text font-lore pl-5">
                    {boss.healthPoints.toLocaleString()} HP
                  </span>
                </div>
              )}
              ...
              {boss.souls && (
                <div className="flex flex-col gap-2">
                  <div className="flex items-center gap-2 text-[10px] uppercase tracking-widest text-ds-muted font-display">
                    <HugeiconsIcon icon={RecordIcon} size={14} className="text-ds-gold" />
                    {t("details.souls")}
                  </div>
                  <span className="text-sm text-ds-text font-lore pl-5">
                    {boss.souls.toLocaleString()}
                  </span>
                </div>
              )}
            </div>
          ...
          {/* <div className="relative h-40 rounded-sm overflow-hidden border border-ds-border-2 opacity-40 group grayscale hover:grayscale-0 transition-all duration-1000">
            ...
            <Image
              ...
              alt="Dark Souls Boss Atmosphere"
              fill
