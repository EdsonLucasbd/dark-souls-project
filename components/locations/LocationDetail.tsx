...
export function LocationDetail() {
  ...
  const state = useEntity<{ data: Location }>("locations", slug as string);

  const isLoading = state.status === "loading" || state.status === "idle";
  const isError = state.status === "error";
  const location = state.status === "success" ? state.data.data : null;
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center gap-6">
        <div className="w-16 h-16 rounded-full border border-ds-border flex items-center justify-center text-ds-muted">
          <HugeiconsIcon icon={Castle01Icon} size={32} />
        </div>
